...
function get_gt(motif::gapped_k_parts_motif, 
                N, raw_data)
    mode_parts = [(1,k) for k = 1:motif.K]
    lens = [i.len for i in motif.P_motifs.P]
    covering = Dict(mp=>Dict{Int16,Int16}() for mp in mode_parts);
    for n = 1:N
        if raw_data[n].mode > 0
            start_ = raw_data[n].motif_where[1];
            end_ = raw_data[n].motif_where[end];
            motif_where = start_:end_;
            str = raw_data[n].str[motif_where];
            upper_on = false; 
            pos = [];
            cur_motif = 1;
            counter = 0;
...
function get_gt(motif::mixture_gapped_k_parts_motifs, 
                N, raw_data)
...
function get_gt(motif::mixture_k_parts_motifs, N, raw_data)
...
function get_gt(motif::single_part_motif, N, raw_data)
    mode_parts = [(1,1)];
    lens = [motif.len];
    covering = Dict(mp=>Dict{Int16,Int16}() for mp in mode_parts);
    for n = 1:N
...
function get_overlaps(gt::ground_truth, ms, data_matrix)
...
function get_overlaps_c(gt::ground_truth, ms, data_matrix)
    L = Int(size(data_matrix,1)/4);
    num_fs = length(ms.pfms);
    gt_overlaps = zeros((gt.num_mp, num_fs)); # true positive 
...
function get_summary(g::good_stuff)
    gt = get_gt(g.gt_motif, g.N, g.raw_data);
    gt_area, _, _, activated_f_area, activated_f_individial, gt_overlaps, bg_overlaps = get_overlaps(gt, g.ms, g.data_matrix)
...
function gt_cover_by(g, target_folder::String)
    ...
    gt_area, gt_area_each, _, 
        activated_f_area, activated_f_individial, 
            gt_overlaps, bg_overlaps = get_overlaps(gt, g.ms, g.data_matrix);
    ...
    y_axis = nothing;
    if typeof(g.gt_motif) == single_part_motif
        y_axis = vcat(["G1", "BG"]);
    elseif typeof(g.gt_motif) == mixture_k_parts_motifs
        y_axis = vcat(["G$i" for i = 1:g.gt_motif.num_modes], ["BG"])
    elseif typeof(g.gt_motif) == gapped_k_parts_motif
        y_axis = vcat(["G$i" for i = 1:g.gt_motif.K], ["BG"])
    elseif typeof(g.gt_motif) ==  mixture_gapped_k_parts_motifs
        y_axis = vcat(["G$i" for i = 1:g.gt_motif.motif.K], ["BG"])
    end
...
function get_perform_coeff(g::good_stuff; digits=3)
    gt = get_gt(g.gt_motif, g.N, g.raw_data);
    gt_area, _, _, _, activated_f_area, _, gt_overlaps, gt_overlaps_c, _ = get_overlaps_c(gt, g.ms, data);
...
function get_perform_coeff_test(g::good_stuff; digits=3)
    gt = get_gt(g.gt_motif, g.N, g.raw_data);
    gt_area, _, _, _, activated_f_area, _, gt_overlaps, gt_overlaps_c, _ = get_overlaps_c(gt, g.ms, data);    
