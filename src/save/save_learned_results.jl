...
function save_found_results_sim(output_folder::String, g::Union{good_stuff, Nothing})        
    if !isnothing(g)
        ...
        make_motif_type_file(g.gt_motif, output_folder);
        ...
        perf_coeff = get_perform_coeff(g);
        println("perf coeff: $perf_coeff")
        ###### save the pickle ###########################
        gms = g.gt_motif; dms = g.ms;
        ...
        run(`python3 $script_loc_render $output_folder $perf_coeff $max_lr_score $(g.prob_per_seq) $script_loc_template`);
    else
